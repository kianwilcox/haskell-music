...
>     modsf sf a (ctx@(CTX _ bbx@((x,y), (w,h)) myid m),sys,inp) = do
>       (l,sys',action,ts,(v,nextSF)) <- expandUISF sf a (CTX TopDown ((x + 4, y + 20), (w - 8, h - 32))
>                                   (pushWidgetID myid) m, sys, inp)
...
It takes a dimension parameter, the number of historical data points to keep 
in the graph at once, the number of pixels to buffer/pad the graph, and a 
color for the graphed line.

> realtimeGraph :: RealFrac a => Dimension -> Int -> Int -> Color -> UISF a ()
> realtimeGraph (w, h) n buffer color = proc i -> do
>   rec lst <- init (take n $ repeat (h-buffer)) -< take n $ (adjust i:lst)
>   canvas (w,h) -< Just $ withColor color $ polyline $ zip xs lst
>   where xs = reverse $ map truncate [0,(fromIntegral w / fromIntegral (n-1))..(fromIntegral w)]
>         adjust i = buffer + truncate (fromIntegral (h - 2*buffer) * (1 - i))

> realtimeGraph' :: RealFrac a => Dimension -> Int -> Int -> Color -> UISF [a] ()
> realtimeGraph' (w, h) n buffer color = proc is -> do
>   rec lst <- init (take n $ repeat (h-buffer)) -< take n $ (reverse (map adjust is) ++ lst)
>   canvas (w,h) -< Just $ withColor color $ polyline $ zip xs lst
>   where xs = reverse $ map truncate [0,(fromIntegral w / fromIntegral (n-1))..(fromIntegral w)]
>         adjust i = buffer + truncate (fromIntegral (h - 2*buffer) * (1 - i))
